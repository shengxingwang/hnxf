<template>
  <view class="uni-file-picker">
    <view v-if="title" class="uni-file-picker__header">
      <text class="file-title">{{ showTitle ? title : '' }}</text>
...
export default {
  ...
  computed: {
    ...
    extname() {
      if (!Array.isArray(this.fileExtname)) {
        const extname = this.fileExtname.replace(/(\[|\])/g, '')
  ...
  methods: {
          ...
          onChooseFile: async res => {
            ...
            if (this.extname && this.extname.length > 0) {
              res.tempFiles.forEach(v => {
                const fileFullName = this.getFileExt(v.name)
    ...
    delFile(index) {
      ...
      /* 为什么要注释下一行代码
      当上传到自己的服务器时，且上传超过2张以上的图片时
      如果随机删掉一张，以下代码会重置file对象，导致剩余的已上传的图片需要重新上传 */
    ...
    async getFileData(files, type = 'image') {
      // 最终需要上传数据库的数据
      const fileFullName = this.getFileExt(files.name)
      const extname = fileFullName.ext.toLowerCase()
      const filedata = {
        name: files.name,
        uuid: files.uuid,
    ...
    backObject(files) {
      const newFilesData = JSON.parse(JSON.stringify(files))
      newFilesData.map(v => {
        delete v.path
        delete v.uuid
        delete v.video
