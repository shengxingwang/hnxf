  ...
  methods: {
    ...
    async validateAll(invalidFields, type, callback) {
      ...
      if (this.validator) {
        for (const i in fieldsValue) {
          ...
          if (resultData) {
            // 获取当前未通过子组件实例
            example = this.childrens.find(
              child => child.name === resultData.key
            )
            // 获取easyInput 组件实例
            const inputComp = this.inputChildrens.find(
              child => child.rename === (example && example.name)
            )
            if (inputComp) {
    ...
    _getValue(key, value) {
      ...
      const isRuleNum = rules.find(
        val => val.format && this.typeFilter(val.format)
      )
